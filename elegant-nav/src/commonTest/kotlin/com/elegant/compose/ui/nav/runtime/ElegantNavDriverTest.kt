...
/**
 * Unit tests for the pure driver logic in ElegantNavDriver.kt:
 * - fingerTarget (gesture progress -> animatedTop target mapping)
 * - elegantNavBackCommitDecision (release commit/cancel decision)
 */
class ElegantNavDriverTest {
    // ---- fingerTarget ----
    ...
    fun fingerTarget_fullProgress_saturatesJustAboveTopMinusOne() {
        ...
        assertEquals(2f - ElegantNavDriverSpec.MAX_FINGER_PROGRESS, fingerTarget(topIndex = 2, progress = 1f))
    }
    ...
    fun fingerTarget_clampsProgressAboveOne() {
        ...
        assertEquals(2f - ElegantNavDriverSpec.MAX_FINGER_PROGRESS, fingerTarget(topIndex = 2, progress = 1.4f))
    }
    ...
    fun anchoredProgress_saturatesJustBelowFullyPopped() {
        ...
        assertEquals(ElegantNavDriverSpec.MAX_FINGER_PROGRESS, anchoredProgress(anchor = 0.5f, fingerProgress = 0.75f))
    }
    ...
    fun anchoredProgress_zeroAnchor_matchesLegacyClamp() {
        ...
        assertEquals(ElegantNavDriverSpec.MAX_FINGER_PROGRESS, anchoredProgress(anchor = 0f, fingerProgress = 1.5f))
    }
    ...
    fun noOvershootFloor_zeroDistance_isZero() {
        ...
        assertEquals(0f, ElegantNavSettleSpec.Spring().commitVelocityFloor(remainingDistance = 0f))
    }

    @Test
    fun noOvershootFloor_isOmegaTimesDistance() {
        // Exact no-overshoot bound for a critically damped spring: v0 >= -ω·x0, ω = √stiffness.
        assertEquals(-sqrt(ElegantNavDriverSpec.STIFFNESS), ElegantNavSettleSpec.Spring().commitVelocityFloor(remainingDistance = 1f), absoluteTolerance = 1e-4f)
    }

    @Test
    fun noOvershootFloor_scalesLinearlyWithDistance() {
        val one = ElegantNavSettleSpec.Spring().commitVelocityFloor(remainingDistance = 1f)
        assertEquals(2f * one, ElegantNavSettleSpec.Spring().commitVelocityFloor(remainingDistance = 2f), absoluteTolerance = 1e-4f)
    }
    ...
    fun noOvershootFloor_negativeDistance_treatedAsZero() {
        ...
        assertEquals(0f, ElegantNavSettleSpec.Spring().commitVelocityFloor(remainingDistance = -0.5f))
    }

    @Test
    fun noOvershootFloor_absentForOptOutAndTween() {
        // clampOvershoot = false (and any tween) removes the floor entirely.
        val unclamped = ElegantNavSettleSpec.Spring(dampingRatio = 0.8f, stiffness = 280f, clampOvershoot = false)
        assertEquals(Float.NEGATIVE_INFINITY, unclamped.commitVelocityFloor(remainingDistance = 1f))
        assertEquals(Float.NEGATIVE_INFINITY, ElegantNavSettleSpec.Tween(450, LinearEasing).commitVelocityFloor(remainingDistance = 1f))
    }

    // ---- elegantNavBackCommitDecision ----
    ...
    fun commit_whenVelocityStronglyTowardPop() {
        ...
        assertEquals(
            true,
            elegantNavBackCommitDecision(progress = 0.1f, velocity = 5f),
        )
    }
    ...
    fun cancel_whenVelocityStronglyBack() {
        ...
        assertEquals(
            false,
            elegantNavBackCommitDecision(progress = 0.8f, velocity = -5f),
        )
    }
    ...
    fun positionFallback_commitsWhenPastHalf() {
        ...
        assertEquals(
            true,
            elegantNavBackCommitDecision(progress = 0.6f, velocity = 0f),
        )
    }
    ...
    fun positionFallback_cancelsWhenBeforeHalf() {
        ...
        assertEquals(
            false,
            elegantNavBackCommitDecision(progress = 0.4f, velocity = 0f),
        )
    }
    ...
    fun positionFallback_commitsExactlyAtHalf() {
        ...
        assertEquals(
            true,
            elegantNavBackCommitDecision(progress = 0.5f, velocity = 0f),
        )
    }
    ...
    fun commit_whenVelocityExactlyAtPositiveThreshold() {
        ...
        assertEquals(
            ...
            elegantNavBackCommitDecision(
                progress = 0.0f,
                velocity = ElegantNavDriverSpec.COMMIT_VELOCITY_THRESHOLD,
            ),
        )
    }
    ...
    // ---- elegantNavBackCompletionDecision ----

    @Test
    fun completion_commitsWithNeutralVelocity() {
        assertEquals(true, elegantNavBackCompletionDecision(velocity = 0f))
    }

    @Test
    fun completion_commitsWithVelocityTowardPop() {
        assertEquals(true, elegantNavBackCompletionDecision(velocity = 5f))
    }
    ...
    fun completion_cancelsAtStrongReturnVelocityThreshold() {
        assertEquals(
            false,
            elegantNavBackCompletionDecision(velocity = -ElegantNavDriverSpec.COMMIT_VELOCITY_THRESHOLD),
        )
    }
}

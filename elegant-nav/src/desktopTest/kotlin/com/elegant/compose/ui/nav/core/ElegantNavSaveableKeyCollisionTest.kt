...
private object FeatureA {
    data class Card(val id: Int) : ElegantNavKey
}
...
private data class HostRoute(val tag: Int) : ElegantNavKey

private data class IntDetail(val id: Int) : ElegantNavKey

private data class StringDetail(val id: String) : ElegantNavKey
...
class ElegantNavSaveableKeyCollisionTest {

    @Test
    fun adjacentToStringCollisionFailsFastWithActionableMessage() = runComposeUiTest {
        val backStack = elegantNavBackStackOf(FeatureA.Card(1))
        setContent {
    ...
    fun separatedToStringCollisionFailsFastInsteadOfSilentStateBleed() = runComposeUiTest {
        ...
        val backStack = elegantNavBackStackOf(FeatureA.Card(7), HostRoute(1), HostRoute(2))
        setContent {
            ElegantNavDisplay(backStack = backStack, effects = ElegantNavDisplayEffects.None) {
                entry<FeatureA.Card> { BasicText("a-card-${it.id}") }
                entry<FeatureB.Card> { BasicText("b-card-${it.id}") }
    ...
    fun collisionWithEntryStillAnimatingOutFailsFast() = runComposeUiTest {
        ...
        val backStack = elegantNavBackStackOf(HostRoute(0), FeatureA.Card(3))
        setContent {
            ElegantNavDisplay(backStack = backStack, effects = ElegantNavDisplayEffects.None) {
                entry<FeatureA.Card> { BasicText("a-card-${it.id}") }
                entry<FeatureB.Card> { BasicText("b-card-${it.id}") }
    ...
    fun factoryKeysCollidingAcrossTypesAreRejected() = runComposeUiTest {
        ...
        val backStack = elegantNavBackStackOf(IntDetail(1))
        setContent {
            ElegantNavDisplay(backStack = backStack, effects = ElegantNavDisplayEffects.None) {

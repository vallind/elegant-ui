 ...
 */
class ElegantNavSettlePhysicsTest {

    @Test
    fun clampedCommit_neverCrossesTarget() {
        val spec = ElegantNavSettleSpec.Spring()
    ...
    fun unclampedUnderdampedCommit_overshootsAndConverges() {
        val spec = ElegantNavSettleSpec.Spring(dampingRatio = 0.8f, stiffness = 280f, clampOvershoot = false)
    ...
    fun settleCancel_pinsRestBoundAndClearsIt() = runBlocking(FixedFrameClock()) {
        ...
        animatable.settleCancel(
            target = 1f,
            spec = ElegantNavSettleSpec.Spring(dampingRatio = 0.6f, stiffness = 280f, clampOvershoot = false),
            initialVelocity = 8f,
        )
    ...
    fun settleProgrammatic_fullStepUsesTween_partialFallsBackToSpring() {
        val motion = ElegantNavMotion(
            commit = ElegantNavSettleSpec.Tween(450, LinearEasing),
            programmatic = ElegantNavSettleSpec.Tween(160, LinearEasing),
        )

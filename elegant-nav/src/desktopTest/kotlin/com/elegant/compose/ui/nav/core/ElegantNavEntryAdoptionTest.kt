...
private data class CoarseDetail(val id: String, val tab: String) : ElegantNavKey

private data class SharedRouteA(val id: String) : ElegantNavKey

private data class SharedRouteB(val id: String) : ElegantNavKey

/**
 * A coarse `contentKey` (e.g. `{ it.id }`) makes reconcile reuse the surviving [ElegantNavEntry] when a
...
class ElegantNavEntryAdoptionTest {

    @Test
    fun replaceUnderSameContentKeyRefreshesRouteArguments() = runComposeUiTest {
        val backStack = elegantNavBackStackOf(CoarseDetail(id = "1", tab = "info"))
        setContent {
    ...
    fun crossTypeReplaceUnderSameContentKeySwapsKeyAndContentAtomically() = runComposeUiTest {
        val backStack = elegantNavBackStackOf(SharedRouteA("7"))
        setContent {
            ElegantNavDisplay(backStack = backStack, effects = ElegantNavDisplayEffects.None) {
        ...
        // Both route types resolve to contentKey "shared-7": the ElegantNavEntry built for SharedRouteA is

...
@Serializable
private sealed interface RestoreRoute : ElegantNavKey
 ...
 * [ElegantNavSaveableStateHolder]. At restore time the stack holds all three slot states at once — the
 ...
 */
@OptIn(ExperimentalTestApi::class)
class ElegantNavStateRestorationTest {

    @Test
    fun rememberSaveableRestoresAcrossMixedRouteHierarchy() = runComposeUiTest {
        val harness = RestorationHarness()
        var stack: ElegantNavBackStack? = null
        setContent {
            harness.Host {
                val backStack = rememberElegantNavBackStack<RestoreRoute>(RestoreHome)
                stack = backStack

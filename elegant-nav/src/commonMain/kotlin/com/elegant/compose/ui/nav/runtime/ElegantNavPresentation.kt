...
import com.elegant.compose.ui.nav.core.ElegantNavEntry
...
/** Mutable [ElegantNavSettle] backing: per-settle constants plus a per-frame elapsed stamp. */
internal class ElegantNavSettleState(
    override val phase: ElegantNavSettlePhase,
    override val releaseVelocity: Float,
) : ElegantNavSettle {
    override var elapsedMillis: Float by mutableFloatStateOf(0f)
}

/** Anything that can hold the published settle context ([ElegantNavPresentation]; a swipe-local holder). */
internal interface ElegantNavSettleSink {
    var settle: ElegantNavSettle?
...
/**
 * Publishes a [ElegantNavSettleState] for the duration of [body], stamping wall-clock elapsed through
 ...
 */
internal suspend fun ElegantNavSettleSink.trackSettle(
    phase: ElegantNavSettlePhase,
    releaseVelocity: Float,
    body: suspend (onFrame: () -> Unit) -> Unit,
) {
    val state = ElegantNavSettleState(phase = phase, releaseVelocity = releaseVelocity)
...
/**
 * Maps a [relativeDepth] to a [ElegantNavRole] (design spec §4.1 / §6.1).
 *
 * - `|d| ≤ TOP_EPSILON` -> [ElegantNavRole.Top] (steady-state top, jitter-tolerant);
 * - `d > TOP_EPSILON` -> [ElegantNavRole.Covered] (below the top, possibly culled by [isVisibleAt]);
 * - `d < -TOP_EPSILON` -> [ElegantNavRole.Outgoing] when [isRemoving], else [ElegantNavRole.Incoming].
 *
 * Boundary ownership: the transition between layers `i` and `i-1` is governed by the upper
 * entry `i`; this classification feeds ElegantNavDisplay's choice of the governing transition (§4.3).
 */
internal fun roleFor(relativeDepth: Float, isRemoving: Boolean): ElegantNavRole = when {
    relativeDepth > TOP_EPSILON -> ElegantNavRole.Covered
    relativeDepth < -TOP_EPSILON -> if (isRemoving) ElegantNavRole.Outgoing else ElegantNavRole.Incoming
    else -> ElegantNavRole.Top
}
 ...
 * is culled here even though [roleFor] would still classify it as [ElegantNavRole.Incoming]/
 * [ElegantNavRole.Outgoing]; its role is simply never acted upon once it is invisible.
 */
 ...
 */
@Stable
internal class ElegantNavPresentation(initialTopIndex: Float) : ElegantNavSettleSink {
    /**
     ...
     * per-frame visuals; both extensions live in `runtime/ElegantNavDriver.kt`.
     */
    ...
    private val _presented = mutableStateListOf<ElegantNavEntry<*>>()
     ...
     */
    val presented: List<ElegantNavEntry<*>> get() = _presented

    /** Last classified stack mutation; surfaced to the transition scope via `change`. */
    var change: ElegantNavChange by mutableStateOf(ElegantNavChange.None)
        private set

    /**
     * Live predictive-back gesture context, surfaced to transitions via
     * [com.elegant.compose.ui.nav.transition.ElegantNavTransitionScope.gesture]. Snapshot state read
     ...
     */
    var gesture: ElegantNavGesture? by mutableStateOf(null)

    /**
     * Threshold-level flag for "a gesture currently owns the float". [gesture] is replaced per
     * event (a fresh [ElegantNavGesture] each move), so composition-time readers must subscribe to this
    ...
    /**
     * Published settle context (see [ElegantNavSettle]); written by [trackSettle] around every
     ...
     */
    override var settle: ElegantNavSettle? by mutableStateOf(null)
     ...
     * set, preserving leaving entries (flagged via [ElegantNavEntry.presentation]'s `isRemoving`) until they
     * are unloaded at relative depth <= -1. [change] is the classification computed by
     * `elegantNavReconcile`. Surviving instances are reused so `movableContentOf` identity is preserved, but they
     * adopt the fresh registration payload ([ElegantNavEntry.adoptFrom]) so a rebuilt entry provider
     * refreshes live entries instead of leaving them pinned to first-push captures.
     *
     * A [ElegantNavChange.None] classification (identical key lists — the reconcile re-ran because the
     ...
     */
    fun reconcile(currentEntries: List<ElegantNavEntry<*>>, change: ElegantNavChange) {
        if (change != ElegantNavChange.None) this.change = change
    ...
    fun unload(entry: ElegantNavEntry<*>) {
        _presented.remove(entry)
    }
}

/**
 * Remembers a [ElegantNavPresentation] across recompositions, seeded with [initialTopIndex].
 *
 ...
 */
@Composable
internal fun rememberElegantNavPresentation(initialTopIndex: Int): ElegantNavPresentation = remember { ElegantNavPresentation(initialTopIndex.toFloat()) }

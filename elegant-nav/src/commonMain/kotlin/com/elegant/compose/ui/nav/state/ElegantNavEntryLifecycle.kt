 ...
 *
 * @param presentation the entry's render-state snapshot; only [ElegantNavPresentationState.isRemoving] is
 ...
 */
internal fun elegantNavMaxLifecycleFor(presentation: ElegantNavPresentationState, d: Float, gestureActive: Boolean): Lifecycle.State = when {
 ...
 * value computed by [elegantNavMaxLifecycleFor] so a covered or removing entry is held below RESUMED.
 *
 * @param maxLifecycle the lifecycle ceiling for this entry (see [elegantNavMaxLifecycleFor]).
 * @return a [LifecycleOwner] scoped to this entry.
 */
@Composable
internal fun rememberElegantNavEntryLifecycleOwner(maxLifecycle: Lifecycle.State): LifecycleOwner = rememberLifecycleOwner(maxLifecycle = maxLifecycle)
 ...
 *
 * @param owner the per-entry lifecycle owner from [rememberElegantNavEntryLifecycleOwner].

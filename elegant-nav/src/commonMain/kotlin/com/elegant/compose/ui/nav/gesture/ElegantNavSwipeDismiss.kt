...
import com.elegant.compose.ui.nav.runtime.ElegantNavSettleSink
import com.elegant.compose.ui.nav.runtime.anchoredProgress
import com.elegant.compose.ui.nav.runtime.commitVelocityFloor
import com.elegant.compose.ui.nav.runtime.elegantNavBackCommitDecision
import com.elegant.compose.ui.nav.runtime.settleCancel
import com.elegant.compose.ui.nav.runtime.settleTo
import com.elegant.compose.ui.nav.runtime.snapToFinger
...
import com.elegant.compose.ui.nav.transition.ElegantNavSwipeEdge
import kotlin.coroutines.coroutineContext
import kotlin.math.abs

/**
 * Interactive swipe-to-dismiss that drives `animatedTop` directly, finger-following, along the axis
 * and direction the current transition declares ([ElegantNavSwipeDirection]).
 *
 * Attached by ElegantNavDisplay to the display container (so an in-flight transition can be grabbed from
 ...
 *
 * [ElegantNavSwipeDirection.None] disables the gesture entirely (pop via back button / system back).
 *
 ...
 *
 * On release the decision is delegated to [elegantNavBackCommitDecision] (**velocity-first,
 * position-fallback**, §7.2), using the shared thresholds [ElegantNavDriverSpec.COMMIT_VELOCITY_THRESHOLD] /
 * [ElegantNavDriverSpec.COMMIT_POSITION_THRESHOLD]. Release velocity is sampled with a [VelocityTracker]
 ...
 * [elegantNavBackCommitDecision] directly. For the convergence spring it is negated to the depth axis
 ...
 * @param direction The dismiss axis + direction; [ElegantNavSwipeDirection.None] disables the gesture.
 * @param animatedTop The single shared depth driver.
 * @param topIndex Current top entry index being peeled away.
 * @param motion Settle physics for the release settles (commit/cancel curves); ElegantNavDisplay resolves
 ...
 * @param onGesture Receives the live [ElegantNavGesture] context per follow event (progress, edge, touch
...
fun Modifier.elegantNavSwipeDismiss(
    enabled: Boolean,
    direction: ElegantNavSwipeDirection,
    animatedTop: Animatable<Float, AnimationVector1D>,
    topIndex: Int,
    motion: ElegantNavMotion = ElegantNavMotion.Default,
    onCommit: () -> Unit,
    onCancel: () -> Unit,
    onGesture: (ElegantNavGesture?) -> Unit = {},
): Modifier = elegantNavSwipeDismissImpl(
    enabled = enabled,
    direction = direction,
...
/**
 * Implementation body shared by the public [elegantNavSwipeDismiss] and ElegantNavDisplay's internal wiring:
 * [settleSink], when present, publishes the release settle as a
 * [com.elegant.compose.ui.nav.transition.ElegantNavSettle] context (the bare public modifier drives and
...
internal fun Modifier.elegantNavSwipeDismissImpl(
    enabled: Boolean,
    direction: ElegantNavSwipeDirection,
    animatedTop: Animatable<Float, AnimationVector1D>,
    topIndex: Int,
    motion: ElegantNavMotion,
    settleSink: ElegantNavSettleSink?,
    externalGestureOwnership: () -> Long = { 0L },
    onCommit: () -> Unit,
    onCancel: () -> Unit,
    onGesture: (ElegantNavGesture?) -> Unit,
): Modifier = composed {
    if (!enabled || direction == ElegantNavSwipeDirection.None) return@composed this
    ...
    val swipeEdge = when (direction) {
        ElegantNavSwipeDirection.LeftToRight -> ElegantNavSwipeEdge.Left
        ElegantNavSwipeDirection.RightToLeft -> ElegantNavSwipeEdge.Right
        else -> ElegantNavSwipeEdge.None
    }

    val isHorizontal = direction == ElegantNavSwipeDirection.LeftToRight || direction == ElegantNavSwipeDirection.RightToLeft
    ...
    val dismissSign = when (direction) {
        ElegantNavSwipeDirection.LeftToRight, ElegantNavSwipeDirection.TopToBottom -> 1f
        ElegantNavSwipeDirection.RightToLeft, ElegantNavSwipeDirection.BottomToTop -> -1f
        ElegantNavSwipeDirection.None -> 0f // unreachable (guarded above)
    }
    ...
    this.pointerInput(enabled, direction, topIndex) {
        ...
        awaitEachGesture {
            ...
            if (elegantNavBackCommitDecision(progress = progress, velocity = progressVelocity)) {
                ...
                scope.launch {
                    ...
                    if (settleSink != null) {
                        settleSink.trackSettle(phase = ElegantNavSettlePhase.Cancel, releaseVelocity = progressVelocity) { onFrame ->
                            settleBody(onFrame)
                        }
                    } else {
                        settleBody(null)

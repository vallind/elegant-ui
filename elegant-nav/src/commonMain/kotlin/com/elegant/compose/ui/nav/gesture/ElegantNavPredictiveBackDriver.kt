...
/**
 * Drives `animatedTop` from a predictive-back [ElegantNavBackEvent] stream, implementing the finger-
 ...
 *   [com.elegant.compose.ui.nav.runtime.elegantNavBackCompletionDecision], and settles with the single
 *   shared spring via [com.elegant.compose.ui.nav.runtime.settleTo], handing off the release
 *   velocity so the snap -> spring boundary is velocity-continuous.
 *
 * Keeping settle/decision out of this function is deliberate: `ElegantPredictiveBackHandler`'s
 * commit/cancel signal comes from the outer try/catch around the stream, not from inside it.
 *
 * @param events Per-gesture cold flow of [ElegantNavBackEvent].
...
internal suspend fun drivePredictiveBack(
    events: Flow<ElegantNavBackEvent>,

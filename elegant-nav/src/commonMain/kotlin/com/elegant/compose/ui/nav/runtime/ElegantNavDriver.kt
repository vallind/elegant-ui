...
import com.elegant.compose.ui.nav.transition.ElegantNavSettleSpec
 ...
 *
 * Clamp range:
 * - upper bound [ElegantNavDriverSpec.MAX_FINGER_PROGRESS], just under the fully-popped end (see the
 ...
 * @return total progress on the pop axis, in `min(anchor, 0f)..`[ElegantNavDriverSpec.MAX_FINGER_PROGRESS].
 */
internal fun anchoredProgress(anchor: Float, fingerProgress: Float): Float = (anchor + fingerProgress).coerceIn(anchor.coerceAtMost(0f), ElegantNavDriverSpec.MAX_FINGER_PROGRESS)
 ...
 * @param progress raw gesture progress; clamped to `0f..`[ElegantNavDriverSpec.MAX_FINGER_PROGRESS] by [anchoredProgress].
 ...
 * (consumed as [ElegantNavSettleSpec] defaults) and the release commit/cancel thresholds (consumed by
 * [elegantNavBackCommitDecision]). Settle curves themselves are per-transition
 * ([com.elegant.compose.ui.nav.transition.ElegantNavMotion]); no other phase redefines these constants.
 */
internal object ElegantNavDriverSpec {
    /**
     ...
     * Overshoot is per-transition opt-in ([ElegantNavSettleSpec.Spring] with `clampOvershoot = false`);
     ...
     * (`t ≈ -ln(threshold) / (damping·√stiffness)`), matching the established elegant navigation
    ...
    /**
     * Duration of the default programmatic full-step tween ([ElegantNavMotion.Default]'s
...
internal fun ElegantNavSettleSpec.toAnimationSpec(): AnimationSpec<Float> = when (this) {
    is ElegantNavSettleSpec.Spring -> androidx.compose.animation.core.spring(
        dampingRatio = dampingRatio,
        stiffness = stiffness,
        visibilityThreshold = ElegantNavDriverSpec.VISIBILITY_THRESHOLD,
    )

    is ElegantNavSettleSpec.Tween -> tween(durationMillis = durationMillis, easing = easing)
}
 ...
 * ([ElegantNavSettleSpec.Spring.clampOvershoot] `= false`) or cannot consume a seed velocity at all
 * ([ElegantNavSettleSpec.Tween]).
 *
 ...
 */
internal fun ElegantNavSettleSpec.commitVelocityFloor(remainingDistance: Float): Float = when {
    this is ElegantNavSettleSpec.Spring && clampOvershoot ->
 ...
 */
internal fun elegantNavBackCommitDecision(
    progress: Float,
    velocity: Float,
    velocityThreshold: Float = ElegantNavDriverSpec.COMMIT_VELOCITY_THRESHOLD,
    positionThreshold: Float = ElegantNavDriverSpec.COMMIT_POSITION_THRESHOLD,
): Boolean = when {
 ...
 */
internal fun elegantNavBackCompletionDecision(
    velocity: Float,
    velocityThreshold: Float = ElegantNavDriverSpec.COMMIT_VELOCITY_THRESHOLD,
 ...
 * from the Animatable's internal velocity) may pass it explicitly. A [ElegantNavSettleSpec.Tween]
...
internal suspend fun Animatable<Float, AnimationVector1D>.settleTo(
    target: Float,
    spec: ElegantNavSettleSpec = ElegantNavMotion.Default.commit,
 ...
 * fixed-duration curve ([ElegantNavMotion.programmatic]) instead of the live spring.
 *
 ...
 */
internal fun usesProgrammaticCurve(velocity: Float, distance: Float): Boolean = velocity == 0f && abs(distance) >= ElegantNavDriverSpec.FULL_STEP_THRESHOLD
 ...
 *
 * - **From rest over a full step** (a programmatic push/pop/multi-pop): [ElegantNavMotion.programmatic]
 ...
 * @param motion the governing transition's settle physics; defaults to [ElegantNavMotion.Default].
 */
internal suspend fun Animatable<Float, AnimationVector1D>.settleProgrammatic(
    ...
    if (usesProgrammaticCurve(velocity = velocity, distance = target - value)) {
        ...
        val spring = motion.commit as? ElegantNavSettleSpec.Spring ?: ElegantNavSettleSpec.Spring()

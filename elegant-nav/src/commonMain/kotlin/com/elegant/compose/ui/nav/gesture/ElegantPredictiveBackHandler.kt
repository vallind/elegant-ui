...
/**
 * Payload emitted by the [ElegantPredictiveBackHandler] progress [Flow].
 *
 * Mirrors the fields of [com.elegant.compose.ui.nav.transition.ElegantNavGesture] so the host can map a
 ...
 *   ESC, Web triggers) report [ElegantNavSwipeEdge.None].
 ...
 *    collects it and snaps `animatedTop` for each [ElegantNavBackEvent].
 ...
 * @param onProgress Suspend block receiving the cold per-gesture [Flow] of [ElegantNavBackEvent].
 ...
 */
@Composable
fun ElegantPredictiveBackHandler(
    enabled: Boolean,
    onProgress: suspend (Flow<ElegantNavBackEvent>) -> Unit,
    onCommit: () -> Unit,
    onCancel: () -> Unit,
...
@Composable
internal fun ElegantPredictiveBackHandlerWithSessions(
    enabled: Boolean,
    onProgress: suspend (sessionId: Long, events: Flow<ElegantNavBackEvent>) -> Unit,
...
internal class NavigationEventFlowAdapter(
    ...
    var currentOnProgress: suspend (sessionId: Long, events: Flow<ElegantNavBackEvent>) -> Unit = { _, _ -> }
    var currentOnCommit: (sessionId: Long?) -> Unit = {}
    var currentOnCancel: (sessionId: Long) -> Unit = {}

    private var channel: Channel<ElegantNavBackEvent>? = null
    ...
    internal fun handleBackStarted(event: NavigationEvent) {
        ...
        val ch = Channel<ElegantNavBackEvent>(capacity = Channel.UNLIMITED)
        channel = ch
...
/** Maps a dispatcher [NavigationEvent] to the common [ElegantNavBackEvent]. */
private fun NavigationEvent.toNavBackEvent(): ElegantNavBackEvent = ElegantNavBackEvent(
    ...
    progress = progress.coerceIn(0f, ElegantNavDriverSpec.MAX_FINGER_PROGRESS),
    swipeEdge = when (swipeEdge) {
        NavigationEvent.EDGE_LEFT -> ElegantNavSwipeEdge.Left
        NavigationEvent.EDGE_RIGHT -> ElegantNavSwipeEdge.Right

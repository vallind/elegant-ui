...
/**
 * Optional thin wrapper over a [SnapshotStateList] of [ElegantNavKey] (i.e. a `ElegantNavBackStack`).
 *
 ...
 */
public class ElegantNavController(public val backStack: SnapshotStateList<ElegantNavKey>) {
    /** Pushes [key] onto the top of the stack. */
    public fun push(key: ElegantNavKey) {
        backStack.add(key)
    }
    ...
    public fun replace(key: ElegantNavKey) {
     ...
     */
    public fun popUntil(predicate: (ElegantNavKey) -> Boolean) {
...
/**
 * Remembers a [ElegantNavController] wrapping a [rememberElegantNavBackStack] seeded with [elements].
 *
 ...
 */
@Composable
public inline fun <reified T : ElegantNavKey> rememberNavController(vararg elements: T): ElegantNavController {
    val backStack = rememberElegantNavBackStack(*elements)
    return remember(backStack) { ElegantNavController(backStack) }
}

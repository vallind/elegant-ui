 ...
 * duration.
 *
 * This reproduces, point for point, the established enter/exit curve of elegant navigation, so
 ...
 */
@Immutable
internal class ElegantNavSettleEasing(
    response: Float,
    damping: Float,
) : Easing {
    private val r: Float
 ...
 * `ElegantNavAnimationEasing` constant the nav3-based elegant navigation ships. The default
 * `ElegantNavMotion.programmatic` curve; public so transition presets can reuse the established curve
 * in custom [com.elegant.compose.ui.nav.transition.ElegantNavSettleSpec.Tween]s.
 */
val ElegantNavProgrammaticEasing: Easing = ElegantNavSettleEasing(response = 0.8f, damping = 0.95f)

 ...
 * and a platform-provided radius ([rememberElegantNavSystemCornerRadius]).
 */
enum class ElegantNavCornerClipMode {
    /**
...
/**
 * Orthogonal visual effects applied by [ElegantNavDisplay] on top of the active
 * [com.elegant.compose.ui.nav.transition.ElegantNavTransition].
 *
 ...
 *   0.dp (no rounding); pass [rememberElegantNavSystemCornerRadius] to follow the device screen corner
 *   (matching the platform), which still yields no rounding where the platform reports 0.
 * @property cornerClipMode which corners to round; [ElegantNavCornerClipMode.Leading] for slide-style
 *   transitions, [ElegantNavCornerClipMode.All] for card-style ones.
 ...
 *   ([com.elegant.compose.ui.nav.transition.ElegantNavTransition.scrimFraction]); this knob only caps
 *   how dark it gets.
 ...
 */
@Immutable
data class ElegantNavDisplayEffects(
    val enableCornerClip: Boolean = true,
    val cornerClipRadius: Dp = 0.dp,
    val cornerClipMode: ElegantNavCornerClipMode = ElegantNavCornerClipMode.Leading,
     ...
     * [enableCornerClip]:
     *
     * - [ElegantNavCornerClipMode.Leading]: only the entry strictly crossing the front edge
     *   (-1 < d < 0); a settled top (d == 0) or fully-exited entry (d == -1) is not clipped.
     * - [ElegantNavCornerClipMode.All]: any layer at a fractional depth (-1 < d < 1, d != 0) — the
    ...
    fun shouldClipCornersAt(relativeDepth: Float): Boolean {
        ...
        return when (cornerClipMode) {
            ElegantNavCornerClipMode.Leading -> relativeDepth > -1f && relativeDepth < 0f
            ElegantNavCornerClipMode.All -> relativeDepth > -1f && relativeDepth < 1f && relativeDepth != 0f
        }
    }
    ...
    companion object {
        ...
        val Default: ElegantNavDisplayEffects = ElegantNavDisplayEffects()

        /** All effects disabled: no corner clip, no dim, no input blocking. */
        val None: ElegantNavDisplayEffects = ElegantNavDisplayEffects(

 ...
 */
internal class ElegantNavEntryViewModelStores : ViewModel() {
...
private val elegantNavStoresFactory = viewModelFactory { initializer { ElegantNavEntryViewModelStores() } }
...
private class DisplayScopedStores : RememberObserver {
    val stores = ElegantNavEntryViewModelStores()
...
/**
 * Remembers the display's [ElegantNavEntryViewModelStores] registry.
 *
 ...
 */
@Composable
internal fun rememberElegantNavEntryViewModelStores(): ElegantNavEntryViewModelStores {
    val parent = LocalViewModelStoreOwner.current
    return if (parent != null) {
        val registryKey = rememberSaveable { "elegant-nav-entry-stores:${Random.nextLong().toString(16)}" }
        remember(parent, registryKey) {
            ViewModelProvider.create(parent, elegantNavStoresFactory)[registryKey, ElegantNavEntryViewModelStores::class]
        }
    } else {
 ...
 */
@Stable
internal class ElegantNavEntryViewModelStoreOwner(
    override val viewModelStore: ViewModelStore,
) : ViewModelStoreOwner

/**
 * Remembers the [ElegantNavEntryViewModelStoreOwner] for the entry identified by [contentKey], backed by
 * the display-level [stores] registry.
 */
@Composable
internal fun rememberElegantNavEntryViewModelStoreOwner(
    stores: ElegantNavEntryViewModelStores,
    contentKey: Any,
): ElegantNavEntryViewModelStoreOwner = remember(stores, contentKey) {
    ElegantNavEntryViewModelStoreOwner(stores.storeFor(contentKey))
}
 ...
 *
 * @param owner the per-entry owner from [rememberElegantNavEntryViewModelStoreOwner].
 * @param content the entry body that resolves ViewModels.
 */
@Composable
internal fun ProvideNavEntryViewModelStore(
    owner: ElegantNavEntryViewModelStoreOwner,

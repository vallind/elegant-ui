...
@JsFun(
    """
        function hideLoading() {
            if (window.__elyonLoading) {
                window.__elyonLoading.finish();
            } else {
...
@JsFun(
    """
        function setFontProgress(done, total) {
            if (window.__elyonLoading && window.__elyonLoading.fontProgress) {
                window.__elyonLoading.fontProgress(done, total);
            }
        }
    """,
)

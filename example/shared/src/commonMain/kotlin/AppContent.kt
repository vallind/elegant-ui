...
import io.elyon.kmp.basic.FloatingToolbarDefaults
import io.elyon.kmp.basic.Icon
import io.elyon.kmp.basic.IconButton
import io.elyon.kmp.basic.NavigationBar
import io.elyon.kmp.basic.NavigationBarDisplayMode
...
import io.elyon.kmp.basic.NavigationRailValue
...
fun AppContent(
    ...
    // elyon-nav back stack. The explicit <Route> supertype is required: a bare
    // rememberNavBackStack(Route.Main) would infer T = the Route.Main singleton type and fail to
    // encode the whole sealed hierarchy for save/restore. elyon-nav is reflection-free, so no
    ...
    val navigationItems = remember {
        listOf(
            NavigationItem(UIConstants.PAGE_TITLES[0], ElyonIcons.Home),
            NavigationItem(UIConstants.PAGE_TITLES[1], ElyonIcons.Create),
            NavigationItem(UIConstants.PAGE_TITLES[2], ElyonIcons.Image),
            NavigationItem(UIConstants.PAGE_TITLES[3], ElyonIcons.Edit),
            NavigationItem(UIConstants.PAGE_TITLES[4], ElyonIcons.Settings),
        )
    }
    ...
    CompositionLocalProvider(
        ...
        // Swipe-to-dismiss is opt-in in elyon-nav and off by default; the example gates it behind a
        ...
        val navTransition = if (isCrossActivityStyle) CrossActivityTransition else NavTransitions.ElyonDefault
...
private fun NavigationBar(
    ...
    AnimatedVisibility(
                        ...
                        if (blurActive) {
                            Modifier.textureBlur(
                                ...
                                colors = BlurDefaults.blurColors(
                                    blendColors = listOf(
                                        BlendColorEntry(color = ElyonTheme.colorScheme.surface.copy(0.8f)),
                                    ),
                                ),
                            )
                        } else {
        ...
        if (appState.useFloatingNavigationBar) {
            val floatingBarColor = if (blurActive) Color.Transparent else ElyonTheme.colorScheme.surfaceContainer
                    ...
                    FloatingNavigationBar(
                        modifier = if (blurActive) {
                            Modifier.textureBlur(
                                ...
                                colors = BlurDefaults.blurColors(
                                    blendColors = listOf(
                                        BlendColorEntry(color = ElyonTheme.colorScheme.surfaceContainer.copy(0.6f)),
                                    ),
                                ),
    ...
    AnimatedVisibility(
        ...
        FloatingToolbar(
            ...
            AnimatedContent(
                ...
                val iconTint = ElyonTheme.colorScheme.onPrimary

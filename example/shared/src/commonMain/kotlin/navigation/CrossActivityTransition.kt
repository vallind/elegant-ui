...
import io.elyon.kmp.nav.transition.navDirectionalTransition
 ...
 *   cancel spring.
 *
 * Lives in the example app — NOT in elyon-nav — as a demonstration that the public customization
 * surface (`navDirectionalTransition` + `navGraphicsTransition` + `NavMotion` + `NavSettle` +
 * [io.elyon.kmp.nav.transition.NavTransitionScope]) is sufficient to express the

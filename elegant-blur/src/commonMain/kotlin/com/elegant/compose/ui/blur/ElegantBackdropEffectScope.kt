...
import com.elegant.compose.ui.blur.internal.COLOR_CONTROLS_SHADER
import com.elegant.compose.ui.blur.internal.MAX_BLUR_TAPS
 ...
 */
sealed interface ElegantBackdropEffectScope :
    Density,
    ElegantRuntimeShaderCache {
     ...
     * raise it. Prefer raising it before any effect that reads it — a raise after [elegantBlur] still
 ...
 */
fun ElegantBackdropEffectScope.elegantColorControls(
    brightness: Float = 0f,
    contrast: Float = 1f,
    saturation: Float = 1f,
) {
 ...
 *
 * @param effect The [RenderEffect] to chain onto [ElegantBackdropEffectScope.renderEffect].
 */
fun ElegantBackdropEffectScope.elegantEffect(effect: RenderEffect) {
    renderEffect = renderEffect.chain(effect)
}
 ...
 *
 * **Pixel-space uniforms must be scaled by [ElegantBackdropEffectScope.downscaleFactor].** When chained
 * after [elegantBlur] (or any effect that raises [ElegantBackdropEffectScope.downscaleFactor]), the backdrop
 ...
 * [ElegantBackdropEffectScope.downscaleFactor] inside [block], otherwise the geometry will sit far
 ...
 */
fun ElegantBackdropEffectScope.elegantRuntimeShaderEffect(
    key: String,
    shaderString: String,
...
internal abstract class ElegantBackdropEffectScopeImpl :
    ElegantBackdropEffectScope,
    ElegantRuntimeShaderCache {
    ...
    var runtimeShaderCache: ElegantRuntimeShaderCache = ElegantRuntimeShaderCacheImpl()
    ...
    // Padded size elegantBlur() built its sampling clamp from in the current apply(); NaN = no blur ran.
    ...
    // elegantBlendColors()/elegantColorControls() build RenderEffects that don't depend on the animating radius,
    ...
    internal var cachedBlendColors: ElegantBlurColors? = null
    ...
    // elegantProgressiveBlur() builds a gradient-scaled variant; cache it on the same inputs plus the
     ...
     * [elegantProgressiveBlur] then only records radii and skips building its (never drawn) loop-shader
    ...
    /**
     * Effects chained before [elegantProgressiveBlur] while [progressiveCompositeActive] — the composite's
    ...
    /**
     * A [elegantBlendColors] call directly following [elegantProgressiveBlur] while [progressiveCompositeActive],
     ...
     */
    internal var progressivePostBlend: ElegantBlurColors? = null
    ...
    /**
     * When >= 0, [elegantBlur] builds at this exact downscale exponent instead of the adaptive choice.
     * The node sets it for the cross-fade lo/hi passes; -1 means auto. Internal — not exposed on
     * the public [ElegantBackdropEffectScope] interface.
     */
    internal var forcedDownscaleExp: Int = -1

    // Cross-fade bracket discovered by the auto [elegantBlur] pass (see computeDownScaleBlend).
    ...
    fun apply(effects: ElegantBackdropEffectScope.() -> Unit) {
        padding = 0f
        renderEffect = null
        downscaleFactor = 1
        noiseCoefficient = 0f
        ...
        // cross-fade bracket) can't inherit a stale bracket from a previous elegantBlur()-based frame.
        blurBlendExpLo = 0
        blurBlendExpHi = 0
        blurBlendFactor = 0f
        ...
        effects()
        // elegantBlur() bakes its sampling clamp from the padding at its call site; if a later effect
...
/** [ElegantBackdropEffectScope] is sealed with only [ElegantBackdropEffectScopeImpl], so this cast is safe. */
internal val ElegantBackdropEffectScope.impl: ElegantBackdropEffectScopeImpl
    get() = this as ElegantBackdropEffectScopeImpl

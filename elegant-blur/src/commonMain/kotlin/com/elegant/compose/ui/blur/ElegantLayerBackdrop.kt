...
/**
 * Creates and remembers a [ElegantLayerBackdrop] that captures content from a [GraphicsLayer].
 *
 * Use [Modifier.elegantLayerBackdrop][elegantLayerBackdrop] on the content container to capture its
 * rendered output, then pass this [ElegantLayerBackdrop] to blur modifiers.
 *
 ...
 */
@Composable
fun rememberElegantLayerBackdrop(
    graphicsLayer: GraphicsLayer = rememberGraphicsLayer(),
    onDraw: ContentDrawScope.() -> Unit = DefaultOnDraw,
): ElegantLayerBackdrop {
    val currentOnDraw by rememberUpdatedState(onDraw)
    return remember(graphicsLayer) {
        ElegantLayerBackdrop(graphicsLayer) { currentOnDraw() }
    }
}

/**
 * A [ElegantBackdrop] that draws from a captured [GraphicsLayer].
 * The layer content is captured via [Modifier.elegantLayerBackdrop][elegantLayerBackdrop].
 *
 * @property graphicsLayer The [GraphicsLayer] holding the captured content drawn as the backdrop.
 */
@Stable
class ElegantLayerBackdrop internal constructor(
    ...
    override fun DrawScope.drawBackdrop(
        ...
        val consumerSize = (density as? ElegantBackdropEffectScope)?.size ?: size

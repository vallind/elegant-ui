...
import com.elegant.compose.ui.blur.internal.BLEND_MODE_SHADER_EXTENDED
import com.elegant.compose.ui.blur.internal.BLEND_MODE_SHADER_EXTENDED_RAMP
import com.elegant.compose.ui.blur.internal.BLEND_MODE_SHADER_STANDARD
import com.elegant.compose.ui.blur.internal.BLEND_MODE_SHADER_STANDARD_RAMP
import com.elegant.compose.ui.blur.internal.BLUR_KERNEL_REACH
import com.elegant.compose.ui.blur.internal.BLUR_RADIUS_TO_SIGMA
import com.elegant.compose.ui.blur.internal.PROGRESSIVE_LEVEL_FRACTION_1
import com.elegant.compose.ui.blur.internal.adjustedVarianceForExp
import com.elegant.compose.ui.blur.internal.chain
import com.elegant.compose.ui.blur.internal.computeDownScaleBlend
import com.elegant.compose.ui.blur.internal.createBlurEffect
import com.elegant.compose.ui.blur.internal.createProgressiveBlurEffect
import com.elegant.compose.ui.blur.internal.downScaleExpFor
import com.elegant.compose.ui.blur.internal.runtimeShaderEffect as createRuntimeShaderEffect

/** Maximum number of blend layers supported by [elegantBlendColors]. Extra entries are dropped. */
internal const val MAX_BLEND_LAYERS = 8

/**
 * Chains a separable Blur into the scope's [ElegantBackdropEffectScope.renderEffect],
 * adjusts [ElegantBackdropEffectScope.padding] to cover the kernel reach, and updates
 * [ElegantBackdropEffectScope.downscaleFactor]. Non-positive radii skip that axis.
 *
 * Raising [ElegantBackdropEffectScope.padding] after [elegantBlur] is safe (the effect block re-runs once
 * with the final padding), but raising it beforehand avoids the rebuild.
 *
 * Typical use:
 * ```
 * Modifier.elegantBackdrop(backdrop, shape = { shape }, effects = {
 *     elegantBlur(20f * density)
 * })
 * ```
 *
 ...
 */
fun ElegantBackdropEffectScope.elegantBlur(radiusX: Float, radiusY: Float = radiusX) {
 ...
 * [ElegantBackdropEffectScope.renderEffect]: a true variable-radius blur whose radius ramps continuously
 * from full to zero along [gradient]. It uses the same adaptive downscale as [elegantBlur], so it never
 ...
 *
 * Paired with `elegantBackdrop`'s `progressiveGradient` (as [Modifier.elegantProgressiveTextureBlur] wires),
 ...
 */
fun ElegantBackdropEffectScope.elegantProgressiveBlur(
    radiusX: Float,
    ...
    // Same adaptive downscale as elegantBlur(): the box prefilter at each level is what keeps the fixed
 ...
 *
 * @param coefficient The noise dithering strength stored in [ElegantBackdropEffectScope.noiseCoefficient].
 *   Values at or below 0 are ignored.
 */
fun ElegantBackdropEffectScope.elegantNoiseDither(coefficient: Float) {
 ...
 * folded into the blend shader's uniforms (separate from any [elegantColorControls] you may
 * have already chained).
 *
 * @param colors The [ElegantBlurColors] whose blend layers and brightness/saturation drive the shader.
 *   An empty blend-layer list is a no-op.
 */
fun ElegantBackdropEffectScope.elegantBlendColors(colors: ElegantBlurColors) {
    ...
    // folded ramp pass instead of chained (see ElegantBackdropEffectScopeImpl.progressivePostBlend).
 ...
 * skips the color conversions, uniform uploads and effect creation. Keyed on the ElegantBlurColors
 * value, so callers that rebuild it (or its list) each frame still hit.
 */
internal fun obtainBlendColorsEffect(scope: ElegantBackdropEffectScopeImpl, colors: ElegantBlurColors): RenderEffect {
...
internal fun buildRampBlendColorsEffect(
    scope: ElegantBackdropEffectScopeImpl,
    colors: ElegantBlurColors,
    ax: Float,
    ay: Float,
    projFull: Float,
    projZero: Float,
...
private fun ElegantBlurColors.needsExtendedBlend(): Boolean {
    val layerCount = minOf(blendColors.size, MAX_BLEND_LAYERS)
    return (0 until layerCount).any { blendColors[it].mode.value >= 100 }
}

private fun RuntimeShader.setBlendLayerUniforms(scope: ElegantBackdropEffectScopeImpl, colors: ElegantBlurColors, needsExtended: Boolean) {
...
/**
 * Runs the standard texture-blur preset chain inside a custom [elegantBackdrop] effect block.
 *
 * Equivalent to what [Modifier.elegantTextureBlur] applies internally:
 * 1. [elegantNoiseDither] for anti-banding
 * 2. [elegantColorControls] for brightness/contrast/saturation in linear (gamma 2.2) space
 * 3. [elegantBlur] with the given radii in dp (multiplied by [ElegantBackdropEffectScope.density])
 * 4. [elegantBlendColors] for the layered tinting
 *
 * Use this to compose the standard preset with additional custom effects:
 * ```
 * Modifier.elegantBackdrop(backdrop, shape = { shape }, effects = {
 *     elegantTextureBlurEffect(blurRadius = 30f, colors = colors)
 ...
 */
fun ElegantBackdropEffectScope.elegantTextureBlurEffect(
    blurRadiusX: Float,
    ...
    val clampedX = blurRadiusX.coerceIn(0f, ElegantBlurDefaults.MaxBlurRadius)
    val clampedY = blurRadiusY.coerceIn(0f, ElegantBlurDefaults.MaxBlurRadius)
    elegantNoiseDither(noiseCoefficient)
    elegantColorControls(colors.brightness, colors.contrast, colors.saturation)
    elegantBlur(clampedX * density, clampedY * density)
    elegantBlendColors(colors)
}

/**
 * Runs the progressive (gradient) texture-blur preset chain inside a custom [elegantBackdrop] effect
 * block: like [elegantTextureBlurEffect] but with [elegantProgressiveBlur] in place of [elegantBlur], so the blur radius
 * ramps from full to zero along [gradient].
 *
 * Pair this with `elegantBackdrop`'s `progressiveGradient` (same [gradient]) so the draw path renders
 * the multi-level composite with a genuinely sharp full-resolution clear end —
 * [Modifier.elegantProgressiveTextureBlur] wires both together.
 *
 ...
 * @param colors Color adjustments and blend layers applied after blur.
 */
fun ElegantBackdropEffectScope.elegantProgressiveTextureBlurEffect(
    blurRadiusX: Float,
    blurRadiusY: Float = blurRadiusX,
    gradient: ElegantProgressiveBlur = ElegantProgressiveBlur.Top,
    ...
    elegantNoiseDither(noiseCoefficient)
    elegantColorControls(colors.brightness, colors.contrast, colors.saturation)
    elegantProgressiveBlur(clampedX * density, clampedY * density, gradient)
    elegantBlendColors(colors)
}

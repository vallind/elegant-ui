...
import com.elegant.compose.ui.blur.highlight.elegantDrawHighlight
import com.elegant.compose.ui.blur.internal.DOWNSAMPLE_2X_SHADER
import com.elegant.compose.ui.blur.internal.DOWNSAMPLE_4X_SHADER
import com.elegant.compose.ui.blur.internal.InverseLayerScope
import com.elegant.compose.ui.blur.internal.NOISE_DITHER_SHADER
import com.elegant.compose.ui.blur.internal.ShapeProvider
import com.elegant.compose.ui.blur.internal.chain
import com.elegant.compose.ui.blur.internal.createProgressiveSharpOverlayEffect
import com.elegant.compose.ui.blur.internal.createProgressiveStackEffect
import com.elegant.compose.ui.blur.internal.progressiveLoopTapBound
import com.elegant.compose.ui.blur.internal.progressiveSharpBandBounds
import com.elegant.compose.ui.blur.internal.progressiveSharpLoopRadius
import com.elegant.compose.ui.blur.internal.recordLayer
import com.elegant.compose.ui.blur.internal.runtimeShaderEffect
 ...
 *
 * @param backdrop The [ElegantBackdrop] providing the background content to capture and blur.
 ...
 *   [ElegantBackdropEffectScope] (e.g. [elegantBlur], [elegantBlendColors], [elegantColorControls]).
 * @param highlight Optional edge highlight resolved against the [ElegantBackdropEffectScope] and painted
 ...
 *   continuously from full to pixel-sharp. Requires a matching [elegantProgressiveBlur] in [effects] (as
 *   [Modifier.elegantProgressiveTextureBlur] wires) — it records the target radii the composite renders;
 ...
 */
fun Modifier.elegantBackdrop(
    backdrop: ElegantBackdrop,
    shape: () -> Shape,
    effects: ElegantBackdropEffectScope.() -> Unit,
    highlight: (ElegantBackdropEffectScope.() -> ElegantHighlight?)? = null,
    ...
    progressiveGradient: ElegantProgressiveBlur? = null,
...
private class DrawBackdropElement(
    val backdrop: ElegantBackdrop,
    val shape: () -> Shape,
    val effects: ElegantBackdropEffectScope.() -> Unit,
    val highlight: (ElegantBackdropEffectScope.() -> ElegantHighlight?)?,
    ...
    val progressiveGradient: ElegantProgressiveBlur? = null,
...
private class DrawBackdropNode(
    var backdrop: ElegantBackdrop,
    val shape: () -> Shape,
    var effects: ElegantBackdropEffectScope.() -> Unit,
    var highlight: (ElegantBackdropEffectScope.() -> ElegantHighlight?)?,
    ...
    var progressiveGradient: ElegantProgressiveBlur? = null,
    ...
    private val effectScope = object : ElegantBackdropEffectScopeImpl() {
    ...
    private var cachedStackGradient: ElegantProgressiveBlur? = null
    private var cachedStackPre: RenderEffect? = null
    private var cachedStackPost: RenderEffect? = null
    private var cachedStackPostBlend: ElegantBlurColors? = null
    ...
    private var sharpOverlayGradient: ElegantProgressiveBlur? = null
    ...
    override fun ContentDrawScope.draw() {
        ...
        highlight?.invoke(effectScope)?.let { resolved ->
            elegantDrawHighlight(
                highlight = resolved,
                shape = effectScope.shape,
                // onAttach cached this from LocalElegantRuntimeShaderCache (a staticCompositionLocalOf, so
    ...
    private fun updateEffects() {
        ...
        // Gradient mode: elegantProgressiveBlur() records radii + downscale and splits the chain; the
        ...
        // Pass 1 (auto): build the lower bracket level into [primary]; elegantBlur() also stashes the
    ...
    /**
     * Composite mode: elegantProgressiveBlur() recorded the level radii + downscale and split the
     * effects chain; assemble the downscaled level-stack effect (pre → levels → folded ramp
     * blend or ramp-masked post branch → dither/noise) into [ElegantBackdropEffectScope.renderEffect]
    ...
    private fun finalizeProgressiveComposite() {
        ...
        if (stack == null) {
            ...
            // elegantProgressiveBlur() falls back to its uniform loop-shader build.
    ...
    private fun chainFullResNoiseIfNeeded() {
        ...
        if (noiseCoeff > 0f && effectScope.downscaleFactor <= 1) {
            ...
            effectScope.elegantRuntimeShaderEffect(
                key = "NoiseDither",
    ...
    override fun onAttach() {
        effectScope.runtimeShaderCache = currentValueOf(LocalElegantRuntimeShaderCache)
        if (enabled) {
            primary.ensureMain()
            observeEffects()
        }

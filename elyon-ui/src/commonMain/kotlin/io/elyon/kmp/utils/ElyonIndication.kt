...
/**
 * Elyon default [Indication] that draws a rectangular overlay when pressed.
 */
@Immutable
class ElyonIndication(
    private val color: Color = Color.Black,
) : IndicationNodeFactory {
    override fun create(interactionSource: InteractionSource): DelegatableNode = ElyonIndicationInstance(interactionSource, color)

    override fun hashCode(): Int = color.hashCode()

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is ElyonIndication) return false
        if (color != other.color) return false
        return true
    }

    private class ElyonIndicationInstance(

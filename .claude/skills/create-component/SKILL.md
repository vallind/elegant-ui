...
description: Create a new Compose Multiplatform UI component for the elyon library, including every required wiring point (library source, example app section + registration, docs interactive demo + registration, EN/zh_CN docs pages, component overview index, VitePress sidebar). Use whenever the user wants to add a new component, create a new composable, scaffold a component, or add a UI element to elyon — even if they only name the component ("add a Chip"). Triggers on "create component", "new component", "add a component", "scaffold component", "new composable", "新建组件", "添加组件", "新组件", "加个组件".
---

# Create a Elyon Component

Add a new component to the elyon library and complete every companion change beyond the component file itself — example registration, docs, sidebar, and so on: 8 touch points in total (see the checklist at the end).
...
| Component type | Reference (under `elyon-ui/src/commonMain/kotlin/io/elyon/kmp/` unless noted) | Key points |
| :--- | :--- | :--- |

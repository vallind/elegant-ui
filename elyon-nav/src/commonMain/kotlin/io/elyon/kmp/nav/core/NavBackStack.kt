 ...
 */
@PublishedApi
internal fun <T : NavKey> navBackStackSaver(elementsSerializer: KSerializer<List<T>>): Saver<NavBackStack, String> = Saver(
    save = { stack ->
        @Suppress("UNCHECKED_CAST")
        NavBackStackJson.encodeToString(elementsSerializer, stack.toList() as List<T>)
    },
    restore = { encoded ->
        val decoded: List<NavKey> = NavBackStackJson.decodeFromString(elementsSerializer, encoded)

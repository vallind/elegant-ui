...
Elyon provides a complete theme system that allows you to easily maintain a consistent design style
throughout your application. The theme system consists of color schemes and text styles.

## Using ElyonTheme

Use `ThemeController` to control the color scheme mode, then wrap your content with `ElyonTheme`:
...
fun App() {
    ...
    ElyonTheme(controller = controller) { /* Content */ }
}
```
...
- **Direct Usage**
  ...
  Provide a color scheme directly to `ElyonTheme(colors = ...)` for full customization without a controller:

  ```kotlin
  @Composable
  fun AppWithColors() {
      val colors = lightColorScheme() // or darkColorScheme()
      ElyonTheme(colors = colors) { /* Content */ }
  }
  ```

## ThemeController

The `ThemeController` manages the current color scheme of the Elyon theme.
...
Access the current theme values through the `ElyonTheme` object:

```kotlin
val colors = ElyonTheme.colorScheme
val textStyles = ElyonTheme.textStyles
val mode = ElyonTheme.colorSchemeMode
val isDynamic = ElyonTheme.isDynamicColor
```

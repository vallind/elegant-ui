...
```kotlin
implementation("io.elyon.kmp:elyon-nav:<version>")
```
...
| Preset | Description |
| :-- | :-- |
| `ElyonDefault` (default) | Full-width slide + quarter-width parallax + light covered alpha falloff |
...
```kotlin
NavDisplay(backStack, transition = NavTransitions.ElyonDefault) {
...
```kotlin
import io.elyon.kmp.nav.transition.navGraphicsTransition
...
NavDisplay(
    ...
    effects = NavDisplayEffects(
        ...
        dimAmount = 0.32f,
        backdropColor = ElyonTheme.colorScheme.background,
    ),
) { /* ... */ }
```
...
Port that approach (or a `SavedStateHandle`-style holder) into your own app layer; it is intentionally left out of `elyon-nav` v1 so the runtime stays a pure depth-driven stack.

## Scope and extension boundaries (v1)

`elyon-nav` v1 is deliberately a **single, flat back stack**. The following are explicitly **not supported in v1** and are deferred to v2+; do not build on them yet:

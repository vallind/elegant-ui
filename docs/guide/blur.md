...
`elyon-blur` is a standalone blur effect library for Compose Multiplatform. It provides backdrop blur, color blending, and texture effects via Modifier extensions. The library supports Android, Desktop (JVM), iOS, macOS, and Web (WasmJs/Js).

::: warning
On Android, `elyon-blur` requires `minSdk` 33 (Android 13) or higher. All
...
Add the `elyon-blur` dependency to your project:

```kotlin
kotlin {
    sourceSets {
        commonMain.dependencies {
            implementation("io.elyon.kmp:elyon-blur:<version>")
        }
    }
}
```
...
```kotlin
dependencies {
    implementation("io.elyon.kmp:elyon-blur-android:<version>")
}
```
...
If your app has a lower `minSdk` than `elyon-blur` and you want to keep
...
```kotlin
import io.elyon.kmp.shader.isRenderEffectSupported
import io.elyon.kmp.shader.isRuntimeShaderSupported
...
```kotlin
import io.elyon.kmp.blur.drawBackdrop
...
import io.elyon.kmp.blur.highlight.LightPosition
import io.elyon.kmp.blur.highlight.LightSource

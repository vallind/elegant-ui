...
Modifier.squircleBorder(
    width = 1.dp,
    color = ElyonTheme.colorScheme.outline,
    cornerRadius = 16.dp,
)
```
...
```kotlin
Modifier.squircleSurface(
    color = ElyonTheme.colorScheme.secondaryVariant,
    topStart = 24.dp,
    topEnd = 24.dp,
    bottomEnd = 0.dp,
    bottomStart = 0.dp,
...
import io.elyon.kmp.squircle.addSquircleRect
...
fun OutlinedSurface(cornerRadius: Dp) {
    ...
    Box(
        Modifier
            .squircleBackground(ElyonTheme.colorScheme.surface, cornerRadius)
            .drawWithContent {
                drawContent()
                path.rewind()
                ...
                drawPath(path, ElyonTheme.colorScheme.outline, style = Stroke(1.dp.toPx()))
            },
    )
}
```

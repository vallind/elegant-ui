...
`elyon-blur` 是一个独立的 Compose Multiplatform 模糊效果库。它通过 Modifier 扩展提供背景模糊、颜色混合和纹理效果。支持 Android、Desktop (JVM)、iOS、macOS 和 Web (WasmJs/Js) 平台。

::: warning 注意
Android 上 `elyon-blur` 要求 `minSdk` 33（Android 13）或更高。模糊、混合、
...
如果你的应用 `minSdk` 低于 `elyon-blur` 但仍想保持单 APK 发布，可以用下面的

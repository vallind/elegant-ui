...
Elyon 提供了一系列工具函数，帮助您更高效地开发应用程序。以下是主要工具函数的详细介绍和使用示例。

## 弹出窗口工具 (ElyonPopupUtils)

`ElyonPopupUtils` 是一个用于显示对话框布局和弹出窗口布局的工具类。该类已经集成到 `Scaffold` 组件中，可以直接使用。
...
Elyon 提供了易于使用的越界回弹效果修饰符，让滚动体验更加流畅自然。
...
`ElyonOverscrollFactory` 实现了 Compose 的 `OverscrollFactory` 接口，与 `overScrollVertical()` 提供完全相同的弹簧物理效果，但以主题级别集成。两个 `ElyonTheme` 重载均已将其设置为默认工厂，因此所有标准 Compose 可滚动组件（`LazyColumn`、`LazyRow`、`verticalScroll`、`horizontalScroll` 等）无需任何额外修饰符即可自动使用 Elyon 越界回弹效果。

```kotlin
// 无需额外修饰符 —— ElyonTheme 已自动提供越界回弹效果
LazyColumn {
...
CompositionLocalProvider(LocalOverscrollFactory provides null) {
    // 此处的组件不会有 Elyon 越界回弹效果
}
```

显式使用 `ElyonOverscrollEffect`（例如与自定义 `Modifier.overscroll()` 配合）：
...
| 特性 | `overScrollVertical()` | `ElyonOverscrollFactory` |
| :--- | :---: | :---: |
...
> **限制说明：** `ElyonOverscrollFactory` 通过接收可滚动组件到达滚动边界后无法消耗的剩余 delta 来工作。如果内容未超出容器高度（例如 `LazyColumn` 的所有条目均能在屏幕内完整显示），可滚动组件不会到达真正的边界，`applyToScroll` 不会收到非零剩余量，越界回弹效果将**无法触发**。此类场景请改用 `overScrollVertical()`。

## 滚动到边界触觉反馈 (Modifier.scrollEndHaptic())

Elyon 提供了用于在可滚动容器快速滑动到其开始或结束边界时触发触觉反馈的修饰符，通过触觉反馈确认已到达边界增强用户的交互体验。
...
Elyon 提供了视觉反馈效果来增强用户交互体验，通过类似触觉的响应提升操作感。

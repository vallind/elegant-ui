...
`NavigationRail` 是 Elyon 中的侧边导航组件，适用于宽屏设备。项目以「图标在上、文字在下」的形式展示，并可通过 `NavigationRailState` 展开为「图标 + 文字」横排的宽布局。
...
| color                      | Color                                  | NavigationRail 的背景颜色                     | ElyonTheme.colorScheme.surface    | 否       |

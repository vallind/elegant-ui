...
prerequisites:
  - 可以在任何地方使用，不需要 `Scaffold` 或 `ElyonPopupHost`
  - 在窗口层级渲染
hostComponent: None
popupHost: None
---
...
`WindowCascadingListPopup` 是支持二级菜单的弹出列表组件，通过 `Dialog` 在窗口层级渲染。与 `OverlayCascadingListPopup` 不同，它不需要 `Scaffold` 或 `ElyonPopupHost`。`DropdownItem.children` 非空的项会成为子菜单触发行；级联深度限制为 **2 级**。

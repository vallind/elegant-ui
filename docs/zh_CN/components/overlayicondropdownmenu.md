...
prerequisites:
  - 必须在 `Scaffold` 中使用以提供 `ElyonPopupHost`
...
::: danger 使用前提
此组件依赖 `Scaffold` 提供的 `ElyonPopupHost` 以显示弹出内容。必须在 `Scaffold` 中使用，否则弹出内容无法正常渲染。
:::
        ...
        SmallTopAppBar(
            ...
            actions = {
                OverlayIconDropdownMenu(entry = entry) {
                    Icon(imageVector = ElyonIcons.Edit, contentDescription = "动作菜单")
                }
            }
        )
    }
...
Scaffold {
    OverlayIconDropdownMenu(entry = entry) {
        Icon(imageVector = ElyonIcons.Sort, contentDescription = "排序")
    }
}
```
...
Scaffold {
    OverlayIconDropdownMenu(entry = entry, collapseOnSelection = false) {
        Icon(imageVector = ElyonIcons.SelectAll, contentDescription = "多选")
    }
}
```
...
Scaffold {
    OverlayIconDropdownMenu(entries = entries) {
        Icon(imageVector = ElyonIcons.MoreCircle, contentDescription = "更多")
    }
}
```

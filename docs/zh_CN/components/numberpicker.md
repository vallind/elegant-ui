...
`NumberPicker` 是 Elyon 中的基础交互组件，用于通过垂直滚动从一组数字中选择值。选中项居中并高亮显示，周围的项目逐渐淡出并缩小。通过 `wrapAround` 参数支持无限循环滚动。
...
| textStyle        | TextStyle          | 选择器项目的文本样式                                 | ElyonTheme.textStyles.title1     | 否       |

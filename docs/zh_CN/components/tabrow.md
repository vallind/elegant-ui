...
`TabRow` 是 Elyon 中的导航组件，用于创建可横向滚动的标签页。提供了标准样式和带轮廓（Contour）样式两种变体，适用于内容分类和导航场景。
...
```kotlin
import io.elyon.kmp.basic.TabRow // 标准样式
import io.elyon.kmp.basic.TabRowWithContour // 带轮廓样式
```

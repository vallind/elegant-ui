...
`TopAppBar` 是 Elyon 中的顶部应用栏组件，用于在界面顶部提供导航、标题和操作按钮。支持大标题模式和普通模式，以及滚动时的动态效果。
        ...
        SmallTopAppBar(
            ...
            navigationIcon = {
                IconButton(onClick = { /* 处理点击事件 */ }) {
                    Icon(ElyonIcons.Back, contentDescription = "返回")
                }
            },
            actions = {
                IconButton(onClick = { /* 处理点击事件 */ }) {
                    Icon(ElyonIcons.More, contentDescription = "更多")
                }
            }
        )
    }
...
| color                      | Color                           | 顶部栏背景颜色                 | ElyonTheme.colorScheme.surface    | 否       |
| titleColor                 | Color                           | 折叠时小标题文字颜色           | ElyonTheme.colorScheme.onSurface  | 否       |
| largeTitle                 | String                          | 大标题文本                     | title                             | 否       |
| largeTitleColor            | Color                           | 展开时大标题文字颜色           | ElyonTheme.colorScheme.onSurface  | 否       |
| subtitle                   | String                          | 显示在标题栏下方的副标题文本   | ""                                | 否       |
| subtitleColor              | Color                           | 副标题文字颜色                 | ElyonTheme.colorScheme.onSurfaceVariantSummary  | 否       |
...
| modifier                   | Modifier                        | 应用于顶部栏的修饰符     | Modifier                          | 否       |
| color                      | Color                           | 顶部栏背景颜色           | ElyonTheme.colorScheme.surface    | 否       |
| titleColor                 | Color                           | 标题文字颜色             | ElyonTheme.colorScheme.onSurface  | 否       |
| subtitle                   | String                          | 显示在标题栏下方的副标题文本 | ""                                | 否       |
| subtitleColor              | Color                           | 副标题文字颜色           | ElyonTheme.colorScheme.onSurfaceVariantSummary  | 否       |
| navigationIcon             | @Composable () -> Unit          | 导航图标区域的可组合函数 | {}                                | 否       |
| actions                    | @Composable RowScope.() -> Unit | 操作按钮区域的可组合函数 | {}                                | 否       |
...
ElyonScrollBehavior 是用于控制顶部栏滚动行为的配置对象。

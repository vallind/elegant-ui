...
`TextField` 是 Elyon 中的基础输入组件，用于接收用户的文本输入。组件提供了丰富的自定义选项，支持标签动画、前置和后置图标等功能。
...
| textStyle             | TextStyle                                    | 输入文本样式           | ElyonTheme.textStyles.main                           | 否       |
...
构造 [TextFieldColors] 实例。按需覆盖任意子集，未指定的参数回退到 Elyon 主题默认值。

...
`ArrowPreference` 是 Elyon 中的箭头指示组件，通常用于导航或展示更多内容。提供了标题、摘要和右侧箭头图标，支持点击交互，常用于设置项、菜单项或列表项中。
...
- 当 `enabled = true` 时使用 `ElyonTheme.colorScheme.onSurfaceVariantActions`。
- 当 `enabled = false` 时使用 `ElyonTheme.colorScheme.disabledOnSecondaryVariant`。

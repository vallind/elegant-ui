...
`Tooltip` 是 Elyon 中用于简短描述锚点元素的组件。参照 Material 3，统一由 `TooltipBox` 将 tooltip 锚定到内容上；你在它的 `tooltip` 槽里填入 `PlainTooltip`（反色表面短标签）或 `RichTooltip`（带可选标题与操作的 surfaceContainer 卡片）。tooltip 在悬停（光标）或长按（触摸）时显示，也可通过 `TooltipState` 以编程方式显示。
...
TooltipBox(
    ...
    IconButton(onClick = { /* ... */ }) {
        Icon(imageVector = ElyonIcons.Basic.Check, contentDescription = "添加到收藏")
    }
}
```
...
`PlainTooltip` 是渲染在 Elyon 反色表面上的短标签、不可交互。它是 `TooltipScope` 扩展，用于 `TooltipBox` 的 `tooltip` 槽内。
...
TooltipBox(
    ...
    IconButton(onClick = { scope.launch { tooltipState.show() } }) {
        Icon(imageVector = ElyonIcons.Basic.Check, contentDescription = "新功能")
    }
}
```
...
向 `PlainTooltip` / `RichTooltip` 传入 `caretShape = TooltipDefaults.caretShape()` 即可绘制指向锚点的箭头（针对 `Above` / `Below` 方位）。默认无箭头，与 Elyon 其余浮层保持一致。

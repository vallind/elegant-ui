...
`Scaffold` 是 Elyon 中的脚手架组件，用于实现基本设计视觉布局结构。它提供了应用程序界面的基本框架，包括顶部栏、底部栏、悬浮按钮等元素的容器。
...
| snackbarHost                 | @Composable () -> Unit              | 用于显示 Snackbar 的容器，Elyon 不提供此组件 | {}                                | 否       |
| popupHost                    | @Composable () -> Unit              | 用于显示弹出窗口的容器                       | \{ ElyonPopupHost() }             | 否       |
| containerColor               | Color                               | 脚手架的背景颜色                             | ElyonTheme.colorScheme.surface | 否       |
...
Scaffold(
    ...
    bottomBar = {
        val items = listOf(
            NavigationItem("首页", ElyonIcons.VerticalSplit),
            NavigationItem("设置", ElyonIcons.Settings)
        )
...
Scaffold(
    ...
    floatingActionButton = {
        FloatingActionButton(
            ...
            Icon(ElyonIcons.Add, contentDescription = "添加")
        }
    },
...
Scaffold(
    ...
    floatingActionButton = {
        FloatingActionButton(
            ...
            Icon(ElyonIcons.Info, contentDescription = "显示消息")
        }
    },
    content = { paddingValues ->
        // 内容区域需要考虑 padding

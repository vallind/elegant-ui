...
`Divider` 是 Elyon 中的基础布局组件，用于在列表和布局中分隔内容。提供了水平分割线和垂直分割线两种形式。
...
```kotlin
import io.elyon.kmp.basic.HorizontalDivider // 水平分割线
import io.elyon.kmp.basic.VerticalDivider  // 垂直分割线
```
...
| DividerColor | Color | 分割线的默认颜色 | ElyonTheme.colorScheme.dividerLine |

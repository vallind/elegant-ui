...
`FloatingToolbar` 是 Elyon 中的悬浮工具栏组件，它将其内容渲染在一个具有圆角背景的容器中，可以水平或垂直排列。实际的屏幕放置由父组件（通常是 `Scaffold`）处理。
...
import io.elyon.kmp.basic.ToolbarPosition // 用于 Scaffold
```
        ...
        FloatingToolbar {
            Row(
                ...
                IconButton(onClick = { /* 操作 1 */ }) {
                    Icon(ElyonIcons.Edit, contentDescription = "编辑")
                }
                IconButton(onClick = { /* 操作 2 */ }) {
                    Icon(ElyonIcons.Delete, contentDescription = "删除")
                }
            }
        }
    },
...
| defaultColor() | @Composable () -> Color | 默认背景颜色   | ElyonTheme.colorScheme.surfaceContainer |
...
FloatingToolbar(
    ...
    Row(
        ...
        IconButton(onClick = { /* 操作 1 */ }) {
            Icon(ElyonIcons.Edit, contentDescription = "编辑", tint = ElyonTheme.colorScheme.onPrimaryContainer)
        }
        IconButton(onClick = { /* 操作 2 */ }) {
            Icon(ElyonIcons.Delete, contentDescription = "删除", tint = ElyonTheme.colorScheme.onPrimaryContainer)
        }
    }
}
```

...
`SliderPreference` is a preference component in Elyon that combines a title/summary with a slider control. The slider is placed in the bottom action area of the `BasicComponent`, making it ideal for settings screens where users need to adjust values such as volume, brightness, or font size.

...
import io.elyon.kmp.basic.PlainTooltip
import io.elyon.kmp.basic.RichTooltip
import io.elyon.kmp.basic.TooltipState
import io.elyon.kmp.basic.rememberTooltipState
import io.elyon.kmp.basic.TooltipDefaults
import io.elyon.kmp.basic.TooltipAnchorPosition
```
...
TooltipBox(text = "Search") {
    IconButton(onClick = { /* ... */ }) {
        Icon(
            imageVector = ElyonIcons.Basic.Search,
...
TooltipBox(
    ...
    IconButton(onClick = { /* ... */ }) {
        Icon(imageVector = ElyonIcons.Basic.Check, contentDescription = "Add to favorites")
    }
}
```
...
`PlainTooltip` is a short, non-interactive label rendered on the Elyon inverse surface. It is a `TooltipScope` extension, so it is used inside the `TooltipBox` `tooltip` slot.
...
TooltipBox(
    ...
    IconButton(onClick = { scope.launch { tooltipState.show() } }) {
        Icon(imageVector = ElyonIcons.Basic.Check, contentDescription = "What's new")
    }
}
```
...
val richColors = TooltipDefaults.richTooltipColors(
    containerColor = ElyonTheme.colorScheme.surfaceContainer,
    contentColor = ElyonTheme.colorScheme.onSurfaceContainerVariant,
    titleContentColor = ElyonTheme.colorScheme.onSurfaceContainer,
    actionContentColor = ElyonTheme.colorScheme.primary,
)
```
...
Pass `caretShape = TooltipDefaults.caretShape()` to `PlainTooltip` / `RichTooltip` to draw a caret pointing at the anchor (for `Above` / `Below` positioning). The default is caret-less, matching the rest of Elyon's floating surfaces.

...
`RangeSliderPreference` is a preference component in Elyon that combines a title/summary with a range slider control. The range slider is placed in the bottom action area of the `BasicComponent`, making it ideal for settings screens where users need to select a range of values such as price filters, frequency bands, or dual-threshold controls.

## Import

```kotlin
import io.elyon.kmp.preference.RangeSliderPreference
```
...
RangeSliderPreference(
    ...
    startAction = {
        Icon(
            imageVector = ElyonIcons.Basic.Audio,

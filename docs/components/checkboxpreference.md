...
`CheckboxPreference` is a checkbox component in Elyon that provides a title, summary, and checkbox
...
```kotlin
import io.elyon.kmp.preference.CheckboxPreference
import io.elyon.kmp.preference.CheckboxLocation
```
...
CheckboxPreference(
    ...
    checkboxColors = CheckboxDefaults.checkboxColors(
        ...
        checkedBackgroundColor = ElyonTheme.colorScheme.secondary
    )
)
```

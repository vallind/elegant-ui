...
`SwitchPreference` is a switch component in Elyon that provides a title, summary, and a switch control on the right. It supports click interaction and is commonly used in settings items and preference toggles.
...
SwitchPreference(
    ...
    switchColors = SwitchDefaults.switchColors(
        checkedThumbColor = ElyonTheme.colorScheme.primary,
        checkedTrackColor = ElyonTheme.colorScheme.primary.copy(alpha = 0.2f)
    )
)
```

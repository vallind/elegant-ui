...
prerequisites:
  - Must be used within `Scaffold` to provide `ElyonPopupHost`
...
::: danger Prerequisite
This component depends on `Scaffold` providing `ElyonPopupHost` to render popup content. It must be used within `Scaffold`, otherwise popup content will not render correctly.
:::
        ...
        SmallTopAppBar(
            ...
            actions = {
                OverlayIconDropdownMenu(entry = entry) {
                    Icon(imageVector = ElyonIcons.Edit, contentDescription = "Action menu")
                }
            }
        )
    }
...
Scaffold {
    OverlayIconDropdownMenu(entry = entry) {
        Icon(imageVector = ElyonIcons.Sort, contentDescription = "Sort")
    }
}
```
...
Scaffold {
    OverlayIconDropdownMenu(entry = entry, collapseOnSelection = false) {
        Icon(imageVector = ElyonIcons.SelectAll, contentDescription = "Multiple selection")
    }
}
```
...
Scaffold {
    OverlayIconDropdownMenu(entries = entries) {
        Icon(imageVector = ElyonIcons.MoreCircle, contentDescription = "More")
    }
}
```

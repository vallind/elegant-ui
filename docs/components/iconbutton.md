...
`IconButton` is a button component in Elyon used for providing auxiliary interaction points. They are typically used in scenarios that require compact buttons, such as toolbars or image lists.
...
IconButton(
    ...
    Icon(
        imageVector = if (isLiked) ElyonIcons.FavoritesFill else ElyonIcons.Favorites,
        contentDescription = if (isLiked) "Like" else "Unlike",
        tint = if (isLiked) Color.Red else ElyonTheme.colorScheme.onBackground
    )
}
```

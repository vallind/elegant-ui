...
`RadioButtonPreference` is a radio button component in Elyon that provides a title, summary, and radio button control. It supports click interactions and is commonly used in single-select settings and selection lists. When an item is selected, its title and summary are tinted with the theme color.
...
```kotlin
import io.elyon.kmp.preference.RadioButtonPreference
```
...
RadioButtonPreference(
    ...
    colors = RadioButtonPreferenceDefaults.radioButtonPreferenceColors(
        ...
        summaryColor = BasicComponentDefaults.summaryColor(
            color = ElyonTheme.colorScheme.secondary
        )
    ),
    selected = selected,

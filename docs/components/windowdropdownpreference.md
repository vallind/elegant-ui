...
`WindowDropdownPreference` is a dropdown menu component in Elyon that provides a title, summary, and a list of dropdown options. It renders at the window level without needing a `Scaffold` host, making it suitable for use cases where `Scaffold` is not available or desired.

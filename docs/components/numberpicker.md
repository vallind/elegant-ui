...
`NumberPicker` is a basic interactive component in Elyon used for selecting a value from a range of numbers by vertical scrolling. The selected item is centered and highlighted, while surrounding items fade out and scale down. Supports infinite scrolling with the `wrapAround` parameter.
...
```kotlin
import io.elyon.kmp.basic.NumberPicker
```
...
| textStyle        | TextStyle          | Text style for picker items                                              | ElyonTheme.textStyles.title1 | No       |

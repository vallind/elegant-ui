...
`Slider` is a basic interactive component in Elyon used for selecting values within a continuous range. Users can adjust values by dragging the slider, making it suitable for scenarios such as volume adjustment, brightness control, and progress display.

Elyon also provides `VerticalSlider` for vertical orientation and `RangeSlider` for selecting a range of values.
...
import io.elyon.kmp.basic.VerticalSlider
import io.elyon.kmp.basic.RangeSlider
```

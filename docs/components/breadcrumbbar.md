...
`BreadcrumbBar` is a horizontal navigation component in Elyon that displays a trail of path segments as capsule-shaped items separated by arrow icons. When the content overflows the available width, it scrolls horizontally instead of collapsing (given that elyon is typically used on mobile devices).
...
BreadcrumbBar(
    ...
    colors = BreadcrumbBarDefaults.breadcrumbBarColors(
        color = ElyonTheme.colorScheme.onBackground.copy(alpha = 0.55f),
        highlightColor = ElyonTheme.colorScheme.primary,
        backgroundColor = ElyonTheme.colorScheme.onBackground.copy(alpha = 0.1f),
        highlightBackgroundColor = ElyonTheme.colorScheme.primary.copy(alpha = 0.2f),
    ),
)
```

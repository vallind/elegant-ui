...
`Icon` is a fundamental icon component in Elyon used to display various vector icons, bitmap icons, or custom drawn content in the interface. It provides multiple ways to render icons to accommodate different icon resource types.

By default, `tint` resolves to `LocalContentColor.current`, so the icon follows the content color provided by parent components such as `Button` or `Surface`. Pass an explicit `Color` (for example `ElyonTheme.colorScheme.onBackground`) when you need to override that, or `Color.Unspecified` to disable tinting and keep the source colors.
...
Elyon Icon supports multiple types of icon resources:
...
Wrap the `Icon` in an `IconButton` to make it clickable while keeping the Elyon touch feedback:
...
IconButton(onClick = { isSelected = !isSelected }) {
    Icon(
        imageVector = if (isSelected) ElyonIcons.FavoritesFill else ElyonIcons.Favorites,
        contentDescription = "Favorites",
    )
}
```
...
val tint by animateColorAsState(
    if (isSelected) ElyonTheme.colorScheme.primary else ElyonTheme.colorScheme.onBackground,
)

Icon(
    painter = rememberVectorPainter(ElyonIcons.Favorites),
    tint = { tint },
    contentDescription = "Favorites",
)
```

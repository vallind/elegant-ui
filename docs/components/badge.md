...
`Badge` is a small status element in Elyon that overlays dynamic information — such as a count of unread messages or pending requests — onto an anchor like an icon. Following Material 3, `BadgedBox` positions a `Badge` relative to its `content`; the badge can be a small dot (icon only) or contain short text.
...
import io.elyon.kmp.basic.BadgeDefaults
```
...
BadgedBox(badge = { Badge() }) {
    Icon(imageVector = ElyonIcons.Settings, contentDescription = "Settings")
}
```
...
object BadgeDefaults {
    ...
    val containerColor: Color
        @Composable get() = ElyonTheme.colorScheme.error

    val contentColor: Color
        @Composable get() = ElyonTheme.colorScheme.onError
}
```
...
| containerColor | Color | Default background color of a badge          | ElyonTheme.colorScheme.error   |
| contentColor   | Color | Default content color of a badge             | ElyonTheme.colorScheme.onError |

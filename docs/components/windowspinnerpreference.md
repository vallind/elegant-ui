...
`WindowSpinnerPreference` is a dropdown selector component in Elyon that provides titles, summaries, and a list of options with icons and text. It renders at the window level without needing a `Scaffold` host, making it suitable for use cases where `Scaffold` is not available or desired.

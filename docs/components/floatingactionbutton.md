...
`FloatingActionButton` is a floating button component in Elyon, typically used to display the most important or frequently used actions on a page. It usually floats above the interface with a prominent visual effect for quick user access.
...
| containerColor             | Color                  | Background color of the button      | ElyonTheme.colorScheme.primary | No       |
...
FloatingActionButton(
    ...
    Icon(
        imageVector = if (expanded) ElyonIcons.Remove else ElyonIcons.Add,

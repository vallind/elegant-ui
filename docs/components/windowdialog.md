...
`WindowDialog` is a window-level dialog component. It renders using platform `Dialog` and does not require `Scaffold` or `ElyonPopupHost`. It supports large-screen optimized animations, system back gesture dismissal, and a composition local to request dismiss from inside content.

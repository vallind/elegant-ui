...
`Snackbar` is a lightweight feedback component in Elyon used to display brief messages at the bottom of the screen. It can optionally provide actions such as “Undo” and supports different display durations.
...
val colors = SnackbarDefaults.snackbarColors(
    containerColor = ElyonTheme.colorScheme.onSecondaryVariant,
    contentColor = ElyonTheme.colorScheme.secondaryVariant,
    actionContentColor = ElyonTheme.colorScheme.onPrimary,
    dismissActionContentColor = ElyonTheme.colorScheme.onSurfaceContainerVariant,

...
private data class SystemPaletteInfo(
    val seedColor: Color,
    val paletteStyle: ElegantThemePaletteStyle,
    val colorSpec: ElegantThemeColorSpec,
)

@Composable
internal actual fun platformDynamicColors(dark: Boolean): ElegantColors {
    val context = LocalContext.current
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
        val paletteInfo = readSystemPaletteInfo(context)
        if (paletteInfo != null) {
            return elegantColorsFromSeed(
...
private fun readSystemPaletteInfo(context: Context): SystemPaletteInfo? {
    return try {
        ...
        val colorSpec = if (Build.VERSION.SDK_INT >= 36) {
            ElegantThemeColorSpec.Spec2025
        } else {
            ElegantThemeColorSpec.Spec2021
        }
...
private fun mapAndroidStyleToThemePaletteStyle(styleName: String): ElegantThemePaletteStyle =
    when (styleName.uppercase()) {
        "TONAL_SPOT" -> ElegantThemePaletteStyle.TonalSpot
        "VIBRANT" -> ElegantThemePaletteStyle.Vibrant
        "EXPRESSIVE" -> ElegantThemePaletteStyle.Expressive
        "SPRITZ" -> ElegantThemePaletteStyle.Neutral
        "RAINBOW" -> ElegantThemePaletteStyle.Rainbow
        "FRUIT_SALAD" -> ElegantThemePaletteStyle.FruitSalad
        "MONOCHROMATIC" -> ElegantThemePaletteStyle.Monochrome
        "MONOCHROME" -> ElegantThemePaletteStyle.Monochrome
        "FIDELITY" -> ElegantThemePaletteStyle.Fidelity
        "CONTENT" -> ElegantThemePaletteStyle.Content
        "NEUTRAL" -> ElegantThemePaletteStyle.Neutral
        else -> ElegantThemePaletteStyle.TonalSpot
    }
...
private fun systemMd3Roles(context: Context, dark: Boolean): ElegantMonetRoles {

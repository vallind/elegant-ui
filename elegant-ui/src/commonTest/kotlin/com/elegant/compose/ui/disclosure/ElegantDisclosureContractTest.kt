...
internal class ElegantDisclosureContractTest {

    @Test
    fun colorsResolveThemeAwareValuesWithValidContrast() {
        val light = resolveDisclosureColors(ElegantLightColors)
        val dark = resolveDisclosureColors(ElegantDarkColors)
    ...
    fun headerStatePrecedenceFollowsDisabledPressedHoveredResting() {
        val colors = resolveDisclosureColors(ElegantLightColors)

        fun header(
            enabled: Boolean = true,
            pressed: Boolean = false,
            hovered: Boolean = false,
        ) = resolveDisclosureHeaderContainer(
            colors = colors,
            enabled = enabled,
            pressed = pressed,
            hovered = hovered,
    ...
    fun interactionTintsStayVisibleInBothThemes() {
        listOf(ElegantLightColors, ElegantDarkColors).forEach { theme ->
            val colors = resolveDisclosureColors(theme)
    ...
    fun defaultsExposeAccessibilityFloorAndSharedAnimationDuration() {
        assertTrue(
            ElegantDisclosureDefaults.MinimumTouchHeight >= 48.dp,
            "headers must keep a 48dp minimum interactive root",
        )
        assertEquals(
            ElegantMotion.standardDurationMillis,
            ElegantDisclosureDefaults.AnimationDurationMillis,
        )
        assertEquals(18.dp, DisclosureChevronSize)
    }

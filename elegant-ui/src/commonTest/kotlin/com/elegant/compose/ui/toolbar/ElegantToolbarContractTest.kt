...
internal class ElegantToolbarContractTest {
    @Test
    fun themeRolesResolveIntoToolbarColors() {
        val resolved = resolveToolbarColors(testThemeColors)

        assertEquals(testThemeColors.backgroundSubtle, resolved.containerColor)
    ...
    fun defaultsRemainStable() {
        assertEquals(48.dp, ElegantToolbarDefaults.Height)
        assertEquals(4.dp, ElegantToolbarDefaults.HorizontalPadding)
        assertEquals(4.dp, ElegantToolbarDefaults.ItemGap)
    }

    @Test
    fun colorsClassSupportsCopyOverrides() {
        val colors = ElegantToolbarColors(

...
- **Semantics**: every interactive component defines role, selected/disabled/error/loading state, and caller-configurable localized state descriptions. Focus treatment must remain visible in Light and Dark themes and must not rely only on color; visual focus rings are opt-in via `ElegantTheme(focusRingEnabled = true)` (default off), and when disabled the focused state is still reported through semantics and the overlay indication while focused visuals fall back to the hovered or resting colors.
